// Build the BOUND Protocol Whitepaper — public/bound-whitepaper.html
//
// Content comes verbatim from scripts/whitepaper-content.json, which is generated
// from BOUND_Whitepaper_v2_July_24_2026.docx. This script assigns presentation only;
// it never rewrites, summarises, or truncates the source text.
//
// The page reuses the BND Token Economic Audit's stylesheet so the two documents are
// visually identical — the CSS is read out of that page rather than duplicated.
...
const doc = JSON.parse(readFileSync(path.join(root, "scripts/whitepaper-content.json"), "utf8"));
...
const form = b => `<div class="formula">${esc(b.text)}</div>`;
...
/* ---------- styles: audit stylesheet + whitepaper-specific additions ---------- */
...
/* The BLI net-asset-value identity, set apart from the prose */
.formula{font-family:var(--mono);font-size:14px;color:var(--accent);background:var(--accentSoft);
  border:1px solid color-mix(in srgb,var(--accent2) 26%,transparent);border-radius:10px;
  padding:16px 18px;margin:18px 0;text-align:center;line-height:1.5;overflow-x:auto}
...
<title>Whitepaper — BOUND Protocol</title>
<style>${rawCss}${wpCss}${PRINT_CSS}</style></head><body>
<div class="layout">
<aside class="sidebar">
  <div class="sb-head">
...
<p class="lede">${esc(f.brand)} — on-chain liquidity infrastructure for tokenized real-world assets.</p>
<div class="metarow">
<div><div class="k">Document</div><div class="v">${esc(f.kind)}</div></div>
<div><div class="k">Version</div><div class="v">${esc(f.version)}</div></div>
<div><div class="k">Protocol</div><div class="v">${esc(f.brand)}</div></div>
<div><div class="k">Origin</div><div class="v">${esc(f.place)}</div></div>
</div></div></div>
<div class="wrap">
${B}
<footer><div class="disc"><b>Note.</b> This page is a faithful transcription of ${esc(f.brand)} ${esc(f.kind)} ${esc(f.version)}. Regenerate via <span class="mono">node scripts/build-whitepaper.mjs</span> after any change to the source document.</div>
...
writeFileSync(path.join(root, "public/bound-whitepaper.html"), shell);
const chapters = doc.chapters.length;
const words = JSON.stringify(doc).split(/\s+/).length;
console.log(`Wrote public/bound-whitepaper.html`);
console.log(`${chapters} chapters · ~${words} words · ${shell.length.toLocaleString()} bytes`);
