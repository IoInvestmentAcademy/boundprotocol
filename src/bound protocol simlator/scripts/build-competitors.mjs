// Build the BOUND Protocol Competitors Analysis — public/bound-competitors-analysis.html
//
// Content comes verbatim from scripts/competitors-content.json, generated from
// BOUND_Competitors_Analysis_July2026.docx by scripts/gen-competitors-content.py (which
// also applies the approved BCI -> BLI and BOUND Reserve -> Bound Liquidity Layer
// corrections). This script assigns presentation only; it never rewrites, summarises,
// or truncates the source text.
//
// Shares the BND Token Economic Audit stylesheet so all four documents match.
...
const doc = JSON.parse(readFileSync(path.join(root, "scripts/competitors-content.json"), "utf8"));
...
<title>Competitors Analysis — BOUND Protocol</title>
...
<p class="lede">${esc(f.brand)} — every live architecture competing to turn locked RWA positions into usable dollars, assessed side by side.</p>
<div class="metarow">
...
<div class="wrap">
${B}
<footer><div class="disc"><b>Note.</b> This page is a faithful transcription of ${esc(f.brand)} ${esc(f.kind)}, ${esc(f.date)}. Regenerate via <span class="mono">node scripts/build-competitors.mjs</span> after any change to the source document.</div>
...
writeFileSync(path.join(root, "public/bound-competitors-analysis.html"), shell);
console.log(`Wrote public/bound-competitors-analysis.html`);
