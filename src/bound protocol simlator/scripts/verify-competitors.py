#!/usr/bin/env python3
"""Prove the Competitors Analysis transcription is lossless.
...
RENAMED = {
    ...
    "BOUND's Reserve -> BOUND's Liquidity Layer": count(r"BOUND's Reserve"),
    'BOUND Reserve -> Bound Liquidity Layer': count(r'(?<!\'s) ?BOUND Reserve'),
}
...
assert "BOUND's Reserve" not in page, 'FAIL: stale "BOUND\'s Reserve" found in rendered page'
print('       no stale "BCI" / "Collateralization" / "BOUND (\'s) Reserve" in the page.')

for h in ['Competitive Framing', 'Direct Competitors', 'Midas', 'Symbiotic Liquid Lane',
          'Ondo Nexus', 'Aave Horizon', 'Indirect Competitors and Structural Alternatives',
          'Issuer-Native Instant Redemption', 'Secondary Venues and Per-Asset Pools',
          'Discounted-Claim and RWA-Backed Dollar Designs', 'RWA-Native Platforms and Chains',
          'Competitive Matrix', 'Strategic Synthesis', 'Works Cited']:
    assert h in page, f'MISSING HEADING: {h}'
print('       all 14 spot-checked headings present.')
