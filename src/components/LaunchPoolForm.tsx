...
export function LaunchPoolForm({
  ...
  const {
    vaultPrice,
    isLoading: isVaultPriceLoading,
    error: vaultPriceError,
  } = useVaultPrice(vault.vault.address, String(vault.vault.address), chainId);
            ...
            <div className="mt-4 p-4 bg-red-50 rounded-lg border border-red-200">
              <div className="flex items-center space-x-2">
                ...
                <p className="text-sm text-red-600">
                  Error fetching vault price: {vaultPriceError.message}
                </p>
              </div>
            </div>
          )}
